...
- **testes** — os 5 testes live do `acervo_fundiario` ganharam o marker `integration_br` (a fonte não respondeu a partir dos runners do GitHub Actions nem dos nós internacionais testados). O CI roda `-m "integration and not integration_br"`; localmente basta `-m integration_br` para exercitá-los
...
### Fixed
- **conab / serie_historica** — a URL do `gergelim` apontava para `graos/girassol/` e retornava 404: o produto constava na lista de `produtos_disponiveis()` mas nunca baixava. Corrigido para `graos/gergelim/`, onde o arquivo existe e parseia normalmente (21 registros, safras 2018/19 a 2024/25)
- **conab / serie_historica** — a área de `cafe`/`cafe_arabica`/`cafe_conilon` vinha da aba `Área em formação` (pés que ainda não produzem): o arquivo tem as abas `Área em produção` e `Área em formação` para a mesma métrica e a última lida sobrescrevia a primeira. Em MG 2025, por exemplo, gravava 319.308 ha (formação) em vez de 1.077.804 ha (produção) — ~5x menor. Agora esses produtos usam explicitamente a aba `Área em produção`, o `cana_area_total` fixa explicitamente a aba `Área Total` (antes dependia da ordem das abas) e uma métrica sobrescrita por outra aba passa a emitir `warning` em vez de acontecer em silêncio
...
- **health** — o estado do health check nunca era persistido desde o run 397: o step usava `actions/cache@v4`, que só salva no post-job de job bem-sucedido, e o script encerrava com `sys.exit(1)` sempre que alguma fonte falhava. Como o `acervo_fundiario` falha em todo run, o job nunca ficava verde e todo `record_check()` era descartado — contadores parados, escalonamento e recuperação não confiáveis. Agora o workflow usa `cache/restore` + `cache/save` com `if: always()` e falha em um step final, depois de gravar o estado
