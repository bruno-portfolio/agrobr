...
### Fixed
- **conab / serie_historica** — o parser sintetizava `ano+1` para cabeçalho de safra com um único ano (`2025` → `2025/26`), inventando um segundo ano que não existe na fonte. Isso corrompia a safra das culturas de ano civil: `cafe`/`cafe_arabica`/`cafe_conilon` (2001+) e `trigo`, `aveia`, `cevada`, `canola`, `centeio`, `triticale`. Agora o ano é preservado como está na planilha (`2025` continua `2025`); produtos com safra de dois anos na fonte (`soja`, `milho`, `arroz`, `feijao`, `algodao`, ...) não mudam. A garantia do contrato `serie_historica_safra` e a regra semântica `safra_format` passaram a aceitar `YYYY` e `YYYY/YY`
